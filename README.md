...
* SRT / VTT，中英界面，浅色 / 深色
* SRT / VTT support, Chinese / English UI, light / dark themes

* 按语义分块，携带前后文与背景设定
* Semantic chunking with surrounding context and background settings

* 可选术语库：关闭后不发送、不提取术语；已保存词条仍留在本地
* Optional glossary: when disabled, glossary data is neither sent nor extracted; previously saved entries remain locally

* 暂停、续译、对照编辑、备份导入导出
* Pause, resume, side-by-side editing, and backup import / export

* 浏览器直连模型，必要时回退到本机转发
* Direct browser-to-model requests, with optional fallback to a local proxy
...
Common environment variables: `LISTEN_HOST`, `LISTEN_PORT`, `UI_LANG` (`en` / `zh-CN`), and `API_HOSTS` (server-side proxy allowlist; does not restrict direct browser connections).
