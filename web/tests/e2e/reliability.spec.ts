...
test('new glossary entries display during RPM waits and keep their identity after pausing', async ({
  ...
  for (const [index, term] of terms.entries()) {
    await expect(page.getByLabel(`术语 ${index + 1} 原文`, { exact: true })).toHaveValue(term.source)
    await expect(page.getByLabel(`术语 ${index + 1} 译法`, { exact: true })).toHaveValue(term.target)
  }
  ...
  await page.getByLabel('术语 1 译法', { exact: true }).fill('现金流（手动修正）')
  await expect.poll(async () => (await savedJob(page)).glossary[0].target).toBe('现金流（手动修正）')
  await page.reload()
  await page.getByRole('tab', { name: /术语与风格/ }).click()
  await expect(page.getByLabel('术语 1 原文', { exact: true })).toHaveValue('cash flow')
  await expect(page.getByLabel('术语 1 译法', { exact: true })).toHaveValue('现金流（手动修正）')
  expect((await savedJob(page)).glossary.map((term: { id: string }) => term.id)).toEqual(ids)
})
...
test('preserves term identity, IME drafts and style conflicts across tabs', async ({ page, context }) => {
  ...
  await other.getByRole('tab', { name: /术语与风格/ }).click()
  const bravo = page.getByLabel('术语 2 译法', { exact: true })
  await bravo.focus()
  ...
  const moved = page.getByLabel('术语 1 译法', { exact: true })
